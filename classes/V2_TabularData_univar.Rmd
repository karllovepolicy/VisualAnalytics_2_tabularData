...
barplot(ToPlot,
         col = paletteHere,
         main=titleText,
         sub=sourceText) # the less ink, the better
```
...
text(x=location,y=ToPlot,labels=plotLabels,
     ...
     cex = 0.9) #cex is the set of the font size
...
location=barplot(ToPlot,
         ...
         main=titleText,
         sub=sourceText,
         cex.names = 0.7, #shrink category names, dispension, below 1 shrink, above 1 enlarge
         xlim=c(0,50), 
         xlab = '(in %)', 
         ylab = 'lala',
         horiz = T)
...
plot3 = plot2 + geom_hline(yintercept = 20, #where
                           linetype="dashed", 
                           size=1.5, #thickness
...
orderBase = ggplot(data = tableFreq, aes(x = reorder(locale, pct), y = pct))
plot6 = orderBase +  geom_bar(fill ="gray", stat = 'identity') +
...
# a new column for color, 2 values for 2 groups 
...
table(eduwa$High.Grade,exclude = 'nothing') # what's the meaning of exclude?
```
...
base + geom_bar(stat = 'identity') #identity means just use the value of the y
```
...
dataHist=hist(eduwa3$Reduced.Lunch) #saving info in dataHist, the base R
```
...
dataHist # how baseR produce the histogram
```
...
eduwa3$redLunchOrd=cut(eduwa3$Reduced.Lunch, # make a new data structure here
...
```{r tapply,  eval=FALSE}
# tapply(variable,group,functionToApply), work at county level, unit analysis
...
# strategy 2: change the original data set
